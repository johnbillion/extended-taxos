...
class Extended_Taxonomy {
	...
	public function __construct( $taxonomy, $object_type, array $args = array(), array $names = array() ) {
		...
		$this->defaults['labels'] = array(
			...
			'search_items'               => isset($names['search_items']) ? $names['search_items'] : sprintf( 'Search %s', $this->tax_plural ),
			'popular_items'              => isset($names['popular_items']) ? $names['popular_items'] : sprintf( 'Popular %s', $this->tax_plural ),
			'all_items'                  => isset($names['all_items']) ? $names['all_items'] : sprintf( 'All %s', $this->tax_plural ),
			'parent_item'                => isset($names['parent_item']) ? $names['parent_item'] : sprintf( 'Parent %s', $this->tax_singular ),
			'parent_item_colon'          => isset($names['parent_item_colon']) ? $names['parent_item_colon'] : sprintf( 'Parent %s:', $this->tax_singular ),
			'edit_item'                  => isset($names['edit_item']) ? $names['edit_item'] : sprintf( 'Edit %s', $this->tax_singular ),
			'view_item'                  => isset($names['view_item']) ? $names['view_item'] : sprintf( 'View %s', $this->tax_singular ),
			'update_item'                => isset($names['update_item']) ? $names['update_item'] : sprintf( 'Update %s', $this->tax_singular ),
			'add_new_item'               => isset($names['add_new_item']) ? $names['add_new_item'] : sprintf( 'Add New %s', $this->tax_singular ),
			'new_item_name'              => isset($names['new_item_name']) ? $names['new_item_name'] : sprintf( 'New %s Name', $this->tax_singular ),
			'separate_items_with_commas' => isset($names['separate_items_with_commas']) ? $names['separate_items_with_commas'] : sprintf( 'Separate %s with commas', $this->tax_plural_low ),
			'add_or_remove_items'        => isset($names['add_or_remove_items']) ? $names['add_or_remove_items'] : sprintf( 'Add or remove %s', $this->tax_plural_low ),
			'choose_from_most_used'      => isset($names['choose_from_most_used']) ? $names['choose_from_most_used'] : sprintf( 'Choose from most used %s', $this->tax_plural_low ),
			'not_found'                  => isset($names['not_found']) ? $names['not_found'] : sprintf( 'No %s found', $this->tax_plural_low ),
			'no_terms'                   => isset($names['no_terms']) ? $names['no_terms'] : sprintf( 'No %s', $this->tax_plural_low ),
			'items_list_navigation'      => isset($names['items_list_navigation']) ? $names['items_list_navigation'] : sprintf( '%s list navigation', $this->tax_plural ),
			'items_list'                 => isset($names['items_list']) ? $names['items_list'] : sprintf( '%s list', $this->tax_plural ),
			'no_item'                    => isset($names['no_item']) ? $names['no_item'] : sprintf( 'No %s', $this->tax_singular_low ), # Custom label
		);
